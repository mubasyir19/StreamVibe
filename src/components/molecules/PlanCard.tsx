...
export default function PlanCard({ title, description, price, unit }: PlanCardProps) {
  return (
    <div id='plan' className='bg-black-10 p-6 border border-black-15 rounded-xl'>
      ...
      <div className='mt-7 grid grid-cols-2 gap-x-3'>
        <button className='bg-black-08 border border-black-15 text-white py-3 px-5 rounded-md font-semibold text-sm'>
          Start Free Trial
        </button>
        <button className='bg-red-45 text-white py-3 px-5 rounded-md font-semibold text-sm'>Choose Plan</button>
      </div>
    </div>
  );
}
